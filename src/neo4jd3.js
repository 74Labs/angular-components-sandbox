...
function Neo4jD3 (_selector, _options) {
  ...
  var container, graph, info, legend, node, nodes, relationship, relationshipOutline, relationshipOverlay, relationshipText, relationships, selector, simulation, svg, svgNodes, svgRelationships, svgScale, svgTranslate,
    classes2colors = {},
    justLoaded = false,
    numClasses = 0,
    options = {
    ...
    function appendNode () {
        return node.enter()
          ...
          .on('click', function (d) {
             d.fx = d.fy = null;
             if (angular.isFunction(options.onNodeClick)) {
               options.onNodeClick(d);
             }
          })
          .on('dblclick', function (d) {
             stickNode(d);
             if (angular.isFunction(options.onNodeDoubleClick)) {
          ...
          .on('mouseenter', function (d) {
             ...
             if (angular.isFunction(options.onNodeMouseEnter)) {
          ...
          .on('mouseleave', function (d) {
             ...
             if (angular.isFunction(options.onNodeMouseLeave)) {
    ...
    function appendOutlineRectToNode (node) {
    ...
    function appendRingRectToNode (node) {
        return node.append('rect')
         .attr('class', 'ring')
         .attr('width', function (d) { return d.w })
         .attr('height', function (d) { return d.h })
    ...
    function appendRelationship () {
      return relationship.enter()
         ...
         .on('dblclick', function (d) {
             if (angular.isFunction(options.onRelationshipDoubleClick)) {
...
//            color = options.colors[Math.min(numClasses, options.colors.length - 1)];
            color = options.colorMap ? options.colorMap[cls] : options.colors[numClasses % options.colors.length];
    ...
    //   return options.colorMap ? (options.colorMap[d.labels[0]] ? options.colorMap[d.labels[0]] : options.colors[options.colors.length * Math.random() << 0]) : defaultColor();
    // }
    ...
    function dragEnded (d) {
        ...
        if (angular.isFunction(options.onNodeDragEnd)) {
    ...
    function dragStarted (d) {
        ...
        if (angular.isFunction(options.onNodeDragStart)) {
    ...
    function image (d) {
        ...
        if (options.images) {
            ...
            if(!imagesForLabel) {
              img = (options.imageUrlPrefix ? options.imageUrlPrefix : '') + d.labels[0].replace(' ', '-') + (options.imageUrlSuffix ? options.imageUrlSuffix : '.svg');
            }
            ...
            if (imagesForLabel) {
                ...
                for (i = 0; i < imagesForLabel.length; i++) {
                    ...
                    if (d.labels[0] === label &&
                        (!property || angular.isDefined(d.properties[property])) &&
    ...
    function init (_selector, _options) {
        ...
        if(options.d3Data) {
          updateWithD3Data(options.d3Data);
            ...
            console.error('Error: d3Data, neo4jData and neo4jDataUrl are empty!');
        }
    }
    ...
    //     Object.keys(options.colorMap).forEach(function (key, index) {
    //         var keys = key.split(','),
    //             value = options.iconMap[key];
    //
    //         keys.forEach(function (key) {
    ...
    function neo4jBoltDataToD3Data (data, mappings) {
        ...
        data.records.forEach(function (rec) {
            ...
            graph.nodes.push({name: angular.toJson(properties, null, '\n'), label: labels, id: id});
        });
        ...
        data.records.forEach(function (rec) {
            ...
            graph.relationships.push({source: nid, target: oid, type: type});
        });
...
*/
    ...
    function toString (d) {
        ...
        Object.keys(d.properties).forEach(function (property) {
            s += ', ' + property + ': ' + angular.toJson(d.properties[property]);
        });
    ...
    function updateInfo (d) {
        ...
        Object.keys(d.properties).forEach(function (property) {
            appendInfoElementProperty('property', property, angular.toJson(d.properties[property]));
        });
    }
...
//        smoothTransform(svgTranslate, svgScale);
    ...
    return {
        ...
        neo4jBoltDataToD3Data: neo4jBoltDataToD3Data,
